...
void UI_Mainwindow::serial_decoder(struct device_settings *d_parms)
{
    ...
    if (d_parms->math_decode_mode == DECODE_MODE_UART) {
        ...
        if (d_parms->wave_mem_view_enabled) {
            ...
            if (d_parms->timebasedelayenable) {
                uart_sample_per_bit = (100.0 / d_parms->timebasedelayscale)
                                      / (double) d_parms->math_decode_uart_baud;
            } else {
        ...
        if (d_parms->math_decode_uart_tx) {
            if (d_parms->chandisplay[d_parms->math_decode_uart_tx
                                     - 1]) // don't try to decode if channel isn't enabled...
            {
                for (i = 1; i < d_parms->wavebufsz; i++) {
                    ...
                    if (!uart_tx_start) {
                        ...
                        {
                            if (d_parms->modelserie == 6) {
                                if (d_parms->wavebuf[d_parms->math_decode_uart_tx - 1][i - 1]
                                    >= d_parms->math_decode_threshold_uart_tx) {
                                    if (d_parms->wavebuf[d_parms->math_decode_uart_tx - 1][i]
                                        < d_parms->math_decode_threshold_uart_tx) {
                                        uart_tx_start = 1;
                    ...
                    } else // uart_rx_start != 0
                    {
                        ...
                        if (++uart_tx_data_bit == d_parms->math_decode_uart_width) {
                            if ((d_parms->math_decode_uart_end)
                                && (d_parms->math_decode_format != 4)) // big endian?
                            {
                            ...
                            d_parms->math_decode_uart_tx_val[d_parms->math_decode_uart_tx_nval]
                                = uart_val;
                            ...
                            if (d_parms->math_decode_uart_par) {
                                ...
                                if (i < d_parms->wavebufsz) {
                                    ...
                                    for (j = 0, uart_parity = 0;
                                         j < d_parms->math_decode_uart_width;
                                         j++) {
                                    ...
                                    if ((uart_parity & 1) != uart_parity_bit) {
                                        d_parms
                                            ->math_decode_uart_tx_err[d_parms->math_decode_uart_tx_nval]
                                            = 1;
                                    }
                                }
                            }
        ...
        if (d_parms->math_decode_uart_rx) {
            if (d_parms->chandisplay[d_parms->math_decode_uart_rx
                                     - 1]) // don't try to decode if channel isn't enabled...
            {
                for (i = 1; i < d_parms->wavebufsz; i++) {
                    ...
                    if (!uart_rx_start) {
                        ...
                        {
                            if (d_parms->modelserie == 6) {
                                if (d_parms->wavebuf[d_parms->math_decode_uart_rx - 1][i - 1]
                                    >= d_parms->math_decode_threshold_uart_rx) {
                                    if (d_parms->wavebuf[d_parms->math_decode_uart_rx - 1][i]
                                        < d_parms->math_decode_threshold_uart_rx) {
                                        uart_rx_start = 1;
                    ...
                    } else // uart_rx_start != 0
                    {
                        ...
                        if (++uart_rx_data_bit == d_parms->math_decode_uart_width) {
                            ...
                            d_parms->math_decode_uart_rx_val[d_parms->math_decode_uart_rx_nval]
                                = uart_val;
                            ...
                            if (d_parms->math_decode_uart_par) {
                                ...
                                if (i < d_parms->wavebufsz) {
                                    ...
                                    if ((uart_parity & 1) != uart_parity_bit) {
                                        d_parms
                                            ->math_decode_uart_rx_err[d_parms->math_decode_uart_rx_nval]
                                            = 1;
                                    }
                                }
                            }
    ...
    if (d_parms->math_decode_mode == DECODE_MODE_SPI) {
        ...
        {
            if (d_parms->timebasedelayenable) {
                spi_timeout = d_parms->math_decode_spi_timeout
        ...
        for (i = 0; i < d_parms->wavebufsz; i++) {
            ...
            if (d_parms->math_decode_spi_mode) // use chip select line?
            {
                ...
                {
                    if (d_parms->wavebuf[d_parms->math_decode_spi_cs - 1][i]
                        < threshold[d_parms->math_decode_spi_cs - 1]) {
                        spi_data_mosi_bit = 0;
                ...
                {
                    if (d_parms->wavebuf[d_parms->math_decode_spi_cs - 1][1]
                        >= threshold[d_parms->math_decode_spi_cs - 1]) {
                        spi_data_mosi_bit = 0;
            ...
            if (d_parms->wavebuf[d_parms->math_decode_spi_clk][i]
                >= threshold[d_parms->math_decode_spi_clk]) {
                spi_clk_new = 1;
            } else {
                spi_clk_new = 0;
            ...
            if (d_parms->math_decode_spi_mosi) {
                if (d_parms->chandisplay[d_parms->math_decode_spi_mosi
                                         - 1]) // don't try to decode if channel isn't enabled...
                {
                    if (d_parms->wavebuf[d_parms->math_decode_spi_mosi - 1][i]
                        >= threshold[d_parms->math_decode_spi_mosi - 1]) {
                    ...
                    if (++spi_data_mosi_bit == d_parms->math_decode_spi_width) {
                        if ((d_parms->math_decode_spi_end)
                            && (d_parms->math_decode_format != 4)) // big endian?
                        {
                        ...
                        d_parms->math_decode_spi_mosi_val[d_parms->math_decode_spi_mosi_nval]
                            = spi_mosi_val;

                        d_parms->math_decode_spi_mosi_val_pos[d_parms->math_decode_spi_mosi_nval]
                            = spi_mosi_bit0_pos;

                        d_parms
                            ->math_decode_spi_mosi_val_pos_end[d_parms->math_decode_spi_mosi_nval++]
                            = i;
            ...
            if (d_parms->math_decode_spi_miso) {
                if (d_parms->chandisplay[d_parms->math_decode_spi_miso
                                         - 1]) // don't try to decode if channel isn't enabled...
                {
                    if (d_parms->wavebuf[d_parms->math_decode_spi_miso - 1][i]
                        >= threshold[d_parms->math_decode_spi_miso - 1]) {
                    ...
                    if (++spi_data_miso_bit == d_parms->math_decode_spi_width) {
                        ...
                        d_parms->math_decode_spi_miso_val[d_parms->math_decode_spi_miso_nval]
                            = spi_miso_val;

                        d_parms->math_decode_spi_miso_val_pos[d_parms->math_decode_spi_miso_nval]
                            = spi_miso_bit0_pos;

                        d_parms
                            ->math_decode_spi_miso_val_pos_end[d_parms->math_decode_spi_miso_nval++]
                            = i;
