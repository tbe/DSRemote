...
UI_decoder_window::UI_decoder_window(QWidget *w_parent)
{
    ...
    connect(threshold_auto_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(threshold_auto_clicked(int)));

    connect(uart_tx_src_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(src_combobox_clicked(int)));
    connect(uart_rx_src_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(src_combobox_clicked(int)));
    connect(uart_polarity_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(uart_polarity_combobox_clicked(int)));
    connect(uart_endian_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(uart_endian_combobox_clicked(int)));
    connect(uart_width_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(uart_width_combobox_clicked(int)));
    connect(uart_stop_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(uart_stop_combobox_clicked(int)));
    connect(uart_parity_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(uart_parity_combobox_clicked(int)));

    connect(spi_clk_src_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(src_combobox_clicked(int)));
    connect(spi_mosi_src_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(src_combobox_clicked(int)));
    connect(spi_miso_src_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(src_combobox_clicked(int)));
    connect(spi_cs_src_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(src_combobox_clicked(int)));

    connect(threshold_1_dspinbox,
            SIGNAL(editingFinished()),
            this,
            SLOT(threshold_1_dspinbox_changed()));
    connect(threshold_2_dspinbox,
            SIGNAL(editingFinished()),
            this,
            SLOT(threshold_2_dspinbox_changed()));
    connect(threshold_3_dspinbox,
            SIGNAL(editingFinished()),
            this,
            SLOT(threshold_3_dspinbox_changed()));
    connect(threshold_4_dspinbox,
            SIGNAL(editingFinished()),
            this,
            SLOT(threshold_4_dspinbox_changed()));
    ...
    connect(spi_timeout_dspinbox,
            SIGNAL(editingFinished()),
            this,
            SLOT(spi_timeout_dspinbox_changed()));
    ...
    connect(spi_select_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(spi_select_combobox_clicked(int)));
    connect(spi_mode_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(spi_mode_combobox_clicked(int)));
    connect(spi_polarity_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(spi_polarity_combobox_clicked(int)));
    connect(spi_edge_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(spi_edge_combobox_clicked(int)));
    connect(spi_endian_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(spi_endian_combobox_clicked(int)));
    connect(format_combobox,
            SIGNAL(currentIndexChanged(int)),
            this,
            SLOT(format_combobox_clicked(int)));
    ...
    if (tabholder->currentIndex() == DECODE_MODE_TAB_UART) {
        if (devparms->modelserie != 1) {
            ...
            devparms->math_decode_threshold[uart_tx_src_combobox->currentIndex() - 1]
                = threshold_1_dspinbox->value();

            snprintf(str,
                     512,
                     ":DEC1:THRE:CHAN%i %e",
                     uart_tx_src_combobox->currentIndex(),
                     devparms->math_decode_threshold[uart_tx_src_combobox->currentIndex() - 1]);
        ...
        if (devparms->modelserie != 1) {
            ...
            snprintf(str,
                     512,
                     ":DEC1:THRE:CHAN%i %e",
                     spi_clk_src_combobox->currentIndex() + 1,
                     devparms->math_decode_threshold[spi_clk_src_combobox->currentIndex()]);
    ...
    if (tabholder->currentIndex() == DECODE_MODE_TAB_UART) {
        if (devparms->modelserie != 1) {
            ...
            devparms->math_decode_threshold[uart_rx_src_combobox->currentIndex() - 1]
                = threshold_2_dspinbox->value();

            snprintf(str,
                     512,
                     ":DEC1:THRE:CHAN%i %e",
                     uart_rx_src_combobox->currentIndex(),
                     devparms->math_decode_threshold[uart_rx_src_combobox->currentIndex() - 1]);
        ...
        if (devparms->modelserie != 1) {
            ...
            snprintf(str,
                     512,
                     ":DEC1:THRE:CHAN%i %e",
                     spi_mosi_src_combobox->currentIndex(),
                     devparms->math_decode_threshold[spi_mosi_src_combobox->currentIndex() - 1]);
    ...
    if (tabholder->currentIndex() == DECODE_MODE_TAB_UART) {
        ...
        if (devparms->modelserie != 1) {
            ...
            devparms->math_decode_threshold[spi_miso_src_combobox->currentIndex() - 1]
                = threshold_3_dspinbox->value();

            snprintf(str,
                     512,
                     ":DEC1:THRE:CHAN%i %e",
                     spi_miso_src_combobox->currentIndex(),
                     devparms->math_decode_threshold[spi_miso_src_combobox->currentIndex() - 1]);
    ...
    if (tabholder->currentIndex() == DECODE_MODE_TAB_UART) {
        ...
        if (devparms->modelserie != 1) {
            ...
            devparms->math_decode_threshold[spi_cs_src_combobox->currentIndex() - 1]
                = threshold_4_dspinbox->value();

            snprintf(str,
                     512,
                     ":DEC1:THRE:CHAN%i %e",
                     spi_cs_src_combobox->currentIndex(),
                     devparms->math_decode_threshold[spi_cs_src_combobox->currentIndex() - 1]);
    ...
    if (tabholder->currentIndex() == DECODE_MODE_TAB_UART) {
        ...
        if (thr_auto == 0) {
            ...
            if (devparms->modelserie != 1) {
                ...
                threshold_1_dspinbox->setValue(
                    devparms->math_decode_threshold[uart_tx_src_combobox->currentIndex() - 1]);
                threshold_2_dspinbox->setValue(
                    devparms->math_decode_threshold[uart_rx_src_combobox->currentIndex() - 1]);
            }
        } else {
        ...
        if (thr_auto == 0) {
            ...
            if (devparms->modelserie != 1) {
                ...
                threshold_2_dspinbox->setValue(
                    devparms->math_decode_threshold[spi_mosi_src_combobox->currentIndex() - 1]);
                threshold_3_dspinbox->setValue(
                    devparms->math_decode_threshold[spi_miso_src_combobox->currentIndex() - 1]);
                threshold_4_dspinbox->setValue(
                    devparms->math_decode_threshold[spi_cs_src_combobox->currentIndex() - 1]);
            }
        } else {
            threshold_1_dspinbox->setValue(0.0);
