...
void UI_Mainwindow::get_deep_memory_waveform()
{
    ...
    for (chn = 0; chn < MAX_CHNS; chn++) {
        ...
        if (devparms.yinc[chn] < 1e-6) {
            errStr = QString("Error, parameter \"YINC\" out of range: %1  line %2 file %3")
                         .arg(devparms.yinc[chn])
                         .arg(__LINE__)
                         .arg(__FILE__);
            goto OUT_ERROR;
        }
        ...
        for (bytes_rcvd = 0; bytes_rcvd < mempnts;) {
            ...
            n = get_data_thrd.get_num_bytes_rcvd();
            if (n < 0) {
                errStr = QString("Can not read from device.  line %i file %s")
                             .arg(__LINE__)
                             .arg(__FILE__);
                goto OUT_ERROR;
            }
            ...
            if (n > SAV_MEM_BSZ) {
                errStr = QString("Datablock too big for buffer: %1  line %2 file %3")
                             .arg(n)
                             .arg(__LINE__)
                             .arg(__FILE__);
                goto OUT_ERROR;
