...
void WaveCurve::paintEvent(QPaintEvent *)
{
    ...
    trig_pos_arrow_pos = (curve_w / 2)
                         - (((devparms->timebaseoffset + devparms->viewer_center_position)
...
void WaveCurve::drawTopLabels(QPainter *painter)
{
    ...
    painter->drawText(140,
                      5,
                      70,
                      20,
                      Qt::AlignCenter,
                      suffixed_metric_value(devparms->timebasescale, 1) + "s");
    ...
    painter->drawText(200,
                      ...
                      20,
                      Qt::AlignCenter,
                      suffixed_metric_value(devparms->samplerate, 0) + "Sa/s");
    ...
    if (devparms->acquirememdepth) {
        ...
        painter->drawText(200,
                          ...
                          20,
                          Qt::AlignCenter,
                          suffixed_metric_value(devparms->acquirememdepth, 1) + "pts");
    } else {
    ...
    painter->drawText(570,
                      5,
                      85,
                      20,
                      Qt::AlignCenter,
                      suffixed_metric_value(devparms->timebaseoffset
                                                + devparms->viewer_center_position,
                                            4)
                          + "s");
    ...
    QString trgLvl = suffixed_metric_value(devparms->triggeredgelevel[devparms->triggeredgesource],
                                           2);
    trgLvl += QString::fromLocal8Bit(
    ...
    if (devparms->math_decode_mode == DECODE_MODE_UART) {
        ...
        if (devparms->math_decode_uart_tx) {
            for (i = 0; i < devparms->math_decode_uart_tx_nval; i++) {
                if ((devparms->math_decode_uart_tx_val_pos[i] >= sample_start)
                    && (devparms->math_decode_uart_tx_val_pos[i] < sample_end)) {
                    painter->fillRect((devparms->math_decode_uart_tx_val_pos[i] - sample_start)
                                          * pix_per_smpl,
                                      line_h_uart_tx - 13,
                                      cell_width,
                                      26,
                    ...
                    painter->drawRect((devparms->math_decode_uart_tx_val_pos[i] - sample_start)
                                          * pix_per_smpl,
                                      line_h_uart_tx - 13,
                                      cell_width,
                                      26);
        ...
        if (devparms->math_decode_uart_rx) {
            for (i = 0; i < devparms->math_decode_uart_rx_nval; i++) {
                if ((devparms->math_decode_uart_rx_val_pos[i] >= sample_start)
                    && (devparms->math_decode_uart_rx_val_pos[i] < sample_end)) {
                    painter->fillRect((devparms->math_decode_uart_rx_val_pos[i] - sample_start)
                                          * pix_per_smpl,
                                      line_h_uart_rx - 13,
                                      cell_width,
                                      26,
                    ...
                    painter->drawRect((devparms->math_decode_uart_rx_val_pos[i] - sample_start)
                                          * pix_per_smpl,
                                      line_h_uart_rx - 13,
                                      cell_width,
                                      26);
        ...
        if (devparms->math_decode_uart_tx) {
            ...
            for (i = 0; i < devparms->math_decode_uart_tx_nval; i++) {
                if ((devparms->math_decode_uart_tx_val_pos[i] >= sample_start)
                    ...
                    {
                        painter->drawText((devparms->math_decode_uart_tx_val_pos[i] - sample_start)
                                              * pix_per_smpl,
                                          line_h_uart_tx - 13,
                                          cell_width,
                                          30,
                                          Qt::AlignCenter,
                                          QStringLiteral("%1")
                                              .arg(devparms->math_decode_uart_tx_val[i],
                                                   2,
                                                   16,
                                                   QLatin1Char('0')));
        ...
        if (devparms->math_decode_uart_rx) {
            ...
            for (i = 0; i < devparms->math_decode_uart_rx_nval; i++) {
                if ((devparms->math_decode_uart_rx_val_pos[i] >= sample_start)
                    ...
                    if (devparms->math_decode_format == 0) // hex
                    {
                        painter->drawText((devparms->math_decode_uart_rx_val_pos[i] - sample_start)
                                              * pix_per_smpl,
                                          line_h_uart_rx - 13,
                                          cell_width,
                                          30,
                                          Qt::AlignCenter,
                                          QStringLiteral("%1")
                                              .arg(devparms->math_decode_uart_rx_val[i],
                                                   2,
                                                   16,
                                                   QLatin1Char('0')));
                    } else if (devparms->math_decode_format == 1) // ASCII
    ...
    if (devparms->math_decode_mode == DECODE_MODE_SPI) {
        ...
        if (devparms->math_decode_spi_mosi) {
            for (i = 0; i < devparms->math_decode_spi_mosi_nval; i++) {
                ...
                painter->fillRect((devparms->math_decode_spi_mosi_val_pos[i] - sample_start)
                                      * pix_per_smpl,
                                  line_h_spi_mosi - 13,
                                  cell_width,
                                  26,
                ...
                painter->drawRect((devparms->math_decode_spi_mosi_val_pos[i] - sample_start)
                                      * pix_per_smpl,
                                  line_h_spi_mosi - 13,
                                  cell_width,
                                  26);
        ...
        if (devparms->math_decode_spi_miso) {
            for (i = 0; i < devparms->math_decode_spi_miso_nval; i++) {
                ...
                painter->fillRect((devparms->math_decode_spi_miso_val_pos[i] - sample_start)
                                      * pix_per_smpl,
                                  line_h_spi_miso - 13,
                                  cell_width,
                                  26,
                ...
                painter->drawRect((devparms->math_decode_spi_miso_val_pos[i] - sample_start)
                                      * pix_per_smpl,
                                  line_h_spi_miso - 13,
                                  cell_width,
                                  26);
        ...
        if (devparms->math_decode_spi_mosi) {
            ...
            case 4:
                ...
                break;
            default:
                painter->drawText(5, line_h_spi_mosi - 35, 80, 30, Qt::AlignCenter, R"(Mosi[???])");
                break;
            }
            ...
            for (i = 0; i < devparms->math_decode_spi_mosi_nval; i++) {
                if (devparms->math_decode_format == 0) // hex
                {
                    ...
                    painter->drawText((devparms->math_decode_spi_mosi_val_pos[i] - sample_start)
                                          * pix_per_smpl,
                                      line_h_spi_mosi - 13,
                                      cell_width,
                                      30,
        ...
        if (devparms->math_decode_spi_miso) {
            ...
            case 4:
                ...
                break;
            default:
                painter->drawText(5, line_h_spi_miso - 35, 80, 30, Qt::AlignCenter, R"(Miso[???])");
                break;
            }
            ...
            for (i = 0; i < devparms->math_decode_spi_miso_nval; i++) {
                if (devparms->math_decode_format == 0) // hex
                {
                    ...
                    painter->drawText((devparms->math_decode_spi_miso_val_pos[i] - sample_start)
                                          * pix_per_smpl,
                                      line_h_spi_miso - 13,
                                      cell_width,
                                      30,
