...
UI_settings_window::UI_settings_window(QWidget *parnt)
{
    ...
    QObject::connect(refreshSpinbox,
                     SIGNAL(valueChanged(int)),
                     this,
                     SLOT(refreshSpinboxChanged(int)));
    QObject::connect(invScrShtCheckbox,
                     SIGNAL(stateChanged(int)),
                     this,
                     SLOT(invScrShtCheckboxChanged(int)));
    QObject::connect(showfpsCheckbox,
                     SIGNAL(stateChanged(int)),
                     this,
                     SLOT(showfpsCheckboxChanged(int)));
    QObject::connect(extendvertdivCheckbox,
                     SIGNAL(stateChanged(int)),
                     this,
                     SLOT(extendvertdivCheckboxChanged(int)));
    QObject::connect(HostLineEdit,
...
void UI_settings_window::extendvertdivCheckboxChanged(int state)
{
    ...
    if (state == Qt::Checked) {
        if ((mainwindow->devparms.connected == 1) && (mainwindow->devparms.math_fft == 1)
            && (mainwindow->devparms.math_fft_split == 0)) {
