...
UI_wave_window::UI_wave_window(struct device_settings *p_devparms,
                               short *wbuf[MAX_CHNS],
                               QWidget *parnt)
{
    int i;
...
void UI_wave_window::wavslider_value_changed(int val)
{
    ...
    devparms->viewer_center_position
        = (double) (((devparms->wavebufsz - (devparms->hordivisions * samples_per_div)) / 2)
                    - devparms->wave_mem_view_sample_start)
...
void UI_wave_window::former_page()
{
    ...
    if (devparms->viewer_center_position
        <= ((((double) devparms->acquirememdepth / devparms->samplerate)
             - (devparms->timebasescale * devparms->hordivisions))
            / -2)) {
        devparms->viewer_center_position = (((double) devparms->acquirememdepth
                                             / devparms->samplerate)
                                            - (devparms->timebasescale * devparms->hordivisions))
                                           / -2;
    }
